...
//   * rule 1: multi-buffer placement — only explicit `placement=bmu` → H;
//     all other placements (auto, static, absent) → S. Static planning
//     (PlanMemory with liveness reuse) is more space-efficient than BMU
//     (slice-quantized); S and BMU share the same physical memory, so S
//     gets space priority and there is no "overflow to BMU".
  ...
  void runOnOperation() override {
    ...
    mod.walk([&](memref::AllocOp alloc) {
      ...
      if (!nAttr) {
        ...
        if (pto::bmuSliceBytes(getAllocScope(alloc)) != 0 &&
            isBranchLocalToIf(alloc))
          alloc->setAttr(pto::kPtoPlanClassAttrName,
                         strAttr(pto::kPtoPlanClassDynamic));
        return;
      }
      // Multi-buffer placement rule: only explicit `placement=bmu` → H; all
      // other placements (auto, static, absent) → S. Static planning uses
      // liveness-based byte-level reuse, far more efficient than BMU's
      // slice-quantized bump allocation (especially for small tiles). S and BMU
      // share the same physical memory, so if S can't fit a buffer, BMU can't
      // either — there is no "overflow to BMU". plan-bmu-layout reserves S
      // space first and carves BMU segments from the remainder.
      auto placementAttr = alloc->getAttrOfType<StringAttr>(
          pto::kPtoMultiBufPlacementAttrName);
      bool hybrid = placementAttr &&
                    placementAttr.getValue() == pto::kPtoMultiBufPlacementBmu;
