...
报告要求工具链输出 `[BISHENG] SIMD VF Function properties for <函数名>: Stack size: <N> bytes` 行。该能力依赖具体 Bisheng 构建，使用前应确认目标工具链能输出该格式。若 on 编译成功但报告不可用，auto 会保留 on 产物，不进行栈大小比较或 off 重编译，并输出 `Warning: Bisheng scheduler auto: SIMD VF stack report unavailable`。这不影响编译失败回退：on 编译失败时仍会重试 off。
