...
int main() {
  ...
  constexpr std::size_t kOutputBytes =
      kElementCount * kOutputCount * sizeof(float);
  ...
  if (!CheckAcl(aclrtMallocHost(&resources.inputHost, kInputBytes),
                "aclrtMallocHost(input)")) {
    return 1;
  }
  if (!CheckAcl(aclrtMallocHost(&resources.outputHost, kOutputBytes),
                "aclrtMallocHost(output)")) {
    return 1;
  }
  if (!CheckAcl(aclrtMalloc(&resources.inputDevice, kInputBytes,
                            ACL_MEM_MALLOC_HUGE_FIRST),
                "aclrtMalloc(input)")) {
    return 1;
  }
  if (!CheckAcl(aclrtMalloc(&resources.outputDevice, kOutputBytes,
                            ACL_MEM_MALLOC_HUGE_FIRST),
                "aclrtMalloc(output)")) {
    return 1;
  }
  ...
  if (!ReadFile("./input.bin", inputFileSize, resources.inputHost,
                kInputBytes)) {
  ...
  if (!ReadFile("./output.bin", outputFileSize, resources.outputHost,
                kOutputBytes)) {
  ...
  if (!CheckAcl(aclrtMemcpy(resources.inputDevice, kInputBytes,
                            resources.inputHost, kInputBytes,
                            ACL_MEMCPY_HOST_TO_DEVICE),
                "aclrtMemcpy(input)")) {
    return 1;
  }
  if (!CheckAcl(aclrtMemcpy(resources.outputDevice, kOutputBytes,
                            resources.outputHost, kOutputBytes,
                            ACL_MEMCPY_HOST_TO_DEVICE),
                "aclrtMemcpy(output)")) {
    return 1;
  }

  LaunchVmulaSharedAcc(static_cast<float *>(resources.inputDevice),
                       static_cast<float *>(resources.outputDevice),
                       resources.stream);
  if (!CheckAcl(aclrtSynchronizeStream(resources.stream),
                "aclrtSynchronizeStream")) {
    return 1;
  }
  if (!CheckAcl(aclrtMemcpy(resources.outputHost, kOutputBytes,
                            resources.outputDevice, kOutputBytes,
